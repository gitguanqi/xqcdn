 ...
 * @LastEditors: fegq
 * @LastEditTime: 2021-10-15 10:47:28
...
XqLightBox.prototype.getImgs = function () {
  ...
  let total = document.getElementById("lightbox-total");
  let that = this;
  if (imgs && imgs.length) {
...
XqLightBox.prototype.addLightBoxElem = function () {
  let htmlStr = `
    <div class="lightbox-title">
        ...
        <div>
            (<strong id="lightbox-current">0</strong>/<strong id="lightbox-total"></strong>) 
            <span class="lightbox-name">【<span id="lightbox-name">图片名称</span>】</span> 
        </div>
        <p>
            <i class="lightbox-scale fa"></i>
            <i class="lightbox-close fa fa-times"></i>
        </p>
    </div>
...
XqLightBox.prototype.addArrow = function () {
  ...
  lightArrowLeft.addEventListener(
    "click",
    function () {
      that.prevImg();
    },
  ...
  lightArrowRight.addEventListener(
    "click",
    function () {
      that.nextImg();
    },
...
XqLightBox.prototype.addIconEvent = function () {
  ...
  let faClose = document.querySelector(".lightbox-close");
  faScale.className = this.isScale
    ? "lightbox-scale fa fa-compress"
    : "lightbox-scale fa fa-arrows-alt";
  let that = this;
  faScale.addEventListener(
    "click",
    function () {
      that.scaleImg();
    },
